...
io.sockets.on('connection', function (socket) {
	...
	socket.on('joinroom', function (joinObj, fn) {
		...
		if(rooms[room] === undefined) {
			...
			if(accepted) {
				//We need to let the server know beforehand so that he starts 
				//to prepare the client template.
				if (fn) {
					fn(true);
				}
				//Add user to room.
	...
	socket.on('users', function() {
		...
		//We need to construct the list since the users in the global 
		//user roster have a reference to socket, which has a reference
